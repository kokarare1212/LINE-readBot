...
def loggedIn(func):
    def checkLogin(*args, **kwargs):
        if args[0].isLogin:
            ...
            args[0].callback.other('関数を呼び出すには、LINEにログインする必要があります')
    return checkLogin
...
class Object(object):
    ...
    def __init__(self):
        if self.isLogin == True:
            self.log("[%s] : ログイン成功" % self.profile.displayName)
    ...
    def updateProfilePicture(self, path, type='p'):
        ...
        if r.status_code != 201:
            raise Exception('プロフィール画像の更新に失敗')
        return True
    ...
    def updateProfileVideoPicture(self, path):
        try:
            ...
            if r_vp.status_code != 201:
                raise Exception('プロフィールのビデオ画像の更新に失敗')
            ...
            raise Exception('pypiからFFmpegとffmpyをインストールする必要があります')
    ...
    def uploadObjSquare(self, squareChatMid, path, type='image', returnAs='bool'):
        ...
        if r.status_code != 201:
            raise Exception('%s のアップロードに失敗.' % type)
        if returnAs == 'bool':
            return True
    ...
    def uploadObjHome(self, path, type='image', returnAs='bool', objId=None):
        ...
        if r.status_code != 201:
            raise Exception('オブジェクトのホームのアップロードに失敗')
        if returnAs == 'objId':
            return objId
        elif returnAs == 'bool':
            return True
    ...
    def forwardObjectMsg(self, to, msgId, contentType='image'):
        ...
        if r.status_code != 200:
            raise Exception('フォワードオブジェクトに失敗')
        return True